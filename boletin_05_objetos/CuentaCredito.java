...
public class CuentaCredito extends Cuenta{
  ...
  private void setCredito(double credito) throws CuentaException {
    if(credito < 0 || credito > CREDITO_MAXIMO) {
      throw new CuentaException("El valor " + credito + " no est� aceptado para el cr�dito.");
    }
  ...
  public void realizarReintegro(double monto) throws CuentaException {
    ...
    if(super.getSaldo() >= monto) {
      ...
      retiro = monto;
    }/* si el saldo del padre es menor que el saldo, entonces hay que comprobar si tiene cr�dito suficiente 
      * es decir: el saldo del padre es mayor o igual que el monto a retirar menos el cr�dito restante
      */
      ...
      //se calcula el cr�dito consumido con la operaci�n
      this.creditoConsumido += monto - super.getSaldo();
    }
    
    /*
     * se realiza el retiro en la clase padre, pero se filtra el valor del 0 para no lanzar excepci�n en el padre...
     */
    if(retiro != 0) {
      ...
      /*
       * Al filtrar el cero no se est� llamando al m�todo del padre, y por tanto no se est� incrementando el n�mero de 
       * reintegros, as� que hace falta llevar un conteo adicional en la clase hija (esta) de los reintegros no registrados
       * en la clase padre.
       */
  ...
  */
  ...
  public String toString() {
    return String.format("Titular: %s.%nSaldo padre: %.2f%nSaldo: %.2f�%nCr�dito total: %.2f%n"
        + "Cr�dito consumido: %.2f%nReintegros realizados: %d%n", this.getTitular(), 
